...
- **비디오 모드 (9단계 입력 + 최종 조립 = 총 10단계)**
  - 상품 연출 광고, 회사 홍보 영상, 감성 일상 브이로그 등 실무 비디오 제작 목적에 맞춤화된 가이드 제공.
...
- **동적 타임라인 빌더**: 비디오 모드의 Step 07(타임라인)에서는 Step 01에서 설정한 전체 영상 길이를 기반으로 장면들을 동적으로 추가, 삭제 및 조절할 수 있습니다. 각 장면의 시간은 전체 예산 범위 내에서 자동으로 계산 및 제한되어 입력 오류를 방지합니다.
